...
def drawResults(dataKey, errorKey=0, DRAW=True):
    deletedNum = 3
    errorList = ['errorsList', 'L1ErrorsList', 'KSErrorsList', 'histEMDErrorsList']
    errorNames = ['EMD', 'L1', 'KS', 'histEMD']
    queryKey = 1
    import pickle
    file = open('output/010-02-2/data{}_query{}.pickle'.format(dataKey, queryKey), 'rb')
    ...
    print('errorsList', len(errorsList), len(errorsList[0]), len(errorsList[0][0]));
    if DRAW == True:
        plt.clf() # 清空画布
        ymax = 0
        ...
        plt.tight_layout()
        plt.savefig('output/010-02-2/data{}_query{}_error{}.pdf'.format(dataKey, queryKey, errorKey))
        plt.show()
