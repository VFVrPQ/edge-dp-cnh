    ...
    if f_dattype== 'REAL':
        ...
        for B in range(0,Bt):
            ...
            for qm in range(0,len(qmins)):
                qmin = qmins[qm]
                zq   = list(filter(lambda X:X>=qmin,q))
                nq   = len(zq)
        ...
        for k in range(0,len(vec)):
            ...
            L.append(-vec[k]*float(sum(t1)) - float(nz)*log(float(zvec[k]) - sum(t2)))
        I = L.index(max(L))
        alpha=vec[I]
        fit = reduce(lambda X,Y: X+[Y+X[-1]],\
                     (list(map(lambda X: pow(X,-alpha)/(float(zvec[I])-sum(list(map(lambda X: pow(X,-alpha),list(map(float,range(1,xmin))))))),range(xmin,xmax+1)))),[0])[1:]
        cdi=[]
        for XM in range(xmin,xmax+1):
            cdi.append(len(list(filter(lambda X: floor(X)<=XM,z)))/float(nz))
        
        gof= max( list(map(lambda X: abs(fit[X] - cdi[X]),range(0,xmax-xmin+1))))
        pz=nz/float(N)
        mmax = 20*xmax
        pdf=[]
        ...
        for B in range(0,Bt):
            ...
            for qm in range(0,len(qmins)):
                qmin = qmins[qm]
                zq    = list(filter(lambda X:X>=qmin,zq))
                nq    = len(zq)
                if nq>1:
                    L=[]
